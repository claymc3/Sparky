...
class checkbutton2:
    def __init__(self, parent, text, onoff, rown, col):
        ...
        self.variable = v
        v.set(onoff)
        b = tk.Checkbutton(parent, highlightthickness = 0, text = text, variable = v)
...
class methyl_dialog(tkutil.Dialog, tkutil.Stoppable):
    ...
    # Primary Methyl HMQC spectrum 
        ...
        self.lf2.pack(fill=tk.X, pady=2)
        self.pmframe= tk.Frame(self.lf2)
        self.pmframe.pack(pady=2)
    ...
    # Type Labeled Methyl HMQC spectrum 
        ...
        self.lf3.pack(fill=tk.X, pady=2)
        self.tmframe= tk.Frame(self.lf3)
        tlb = tkutil.entry_field(self.tmframe, 'Selected Methyls: ', '', 8)
        tlb.frame.grid(row=0, column = 0,sticky = tk.W, columnspan = 2)
        self.typelabeling = tlb.variable
        self.Type_HMQC_2D = HMQC_spectrum_menu(session, self.tmframe, 'Spectrum: ')
        self.Type_HMQC_2D.frame.grid(row=0,column=2,columnspan = 4)
        # self.Type_HMQC_2D.frame.pack(side=tk.LEFT,padx=3, expand = 1)
        l2 = tk.Label(self.tmframe, text = 'Stereospecific?: ')
        l2.grid(row = 1, column = 0, pady=2)
        tpr = checkbutton2(self.tmframe, 'proR (D1/G1)', 0,1,1)
        self.TproR = tpr
        tps = checkbutton2(self.tmframe, 'proS (D2/G2)', 0,1,2)
        self.TproS = tps

        self.tmframe.pack(pady=2)

    # Select Cm-CmHm NOESY Spectrum 
        self.sc_CCH_NOESY = spectrum_menu_3D(session, self.top, 'Cm-CmHm NOESY Spectrum: ')
        self.sc_CCH_NOESY.frame.pack(side = 'top', anchor = 'w',padx=2, pady=2)

    # Select HMBC-HMQC 3D Spectrum
        self.sc_HMBC = spectrum_menu_3D(session, self.top, 'HMBC-HMQC Spectrum: ')
        self.sc_HMBC.frame.pack(side = 'top', anchor = 'w',padx=2, pady=2)

    # Select Short mixing Cm-CmHm NOESY Spectrum 
        self.sc_shortCCH_NOESY = spectrum_menu_3D(session, self.top, 'Short mixing Cm-CmHm NOESY Spectrum: ')
        self.sc_shortCCH_NOESY.frame.pack(side = 'top', anchor = 'w',padx=2, pady=2)

    # PDB Information 
        self.lf1 = tk.LabelFrame(self.top, text='PDB')
        self.lf1.pack(fill=tk.X, pady=2)
        self.pdbframe= tk.Frame(self.lf1)
        self.pdbframe.pack(pady=2)
    ...
    # Type of Methyl Labeling 
        self.lf2 = tk.LabelFrame(self.top, text='LV Labeling')
        self.lf2.pack(fill=tk.X, pady=2)
        self.LVframe = tk.Frame(self.lf2)
        di = checkbutton2(self.LVframe, 'Dimethyl', 1,0,0)
        self.dimethyl = di
        im = checkbutton2(self.LVframe, 'Mono-methyl', 0,0,1)
        self.mono = im
        ps = checkbutton2(self.LVframe, 'proS (D2/G2)', 0,0,2)
        self.proS = ps
        pr = checkbutton2(self.LVframe, 'proR (D1/G1)', 0,0,3)
        self.proR = pr
        self.LVframe.pack(pady=2)
    # Rename Resonances option 
        rename = tkutil.checkbutton(self.top, 'Keep Existing Assignments', 1)
        rename.button.pack(side = 'top', anchor = 'w',padx=2, pady=2)
        self.rename = rename
    ...
    # PPM Tolerances for filtering noesy and finding geminal pairs
        er = tkutil.entry_row(self.top, 'PPM tolerance: ',
                                ('1H', '0.01', 5),
                                ('13C', '0.10', 5))
        er.frame.pack(side = 'top', anchor = 'w',padx=2,pady=2)
        self.ppm_range = er

        progress_label = tk.Label(self.top, anchor = 'nw')
        progress_label.pack(side = 'top', anchor = 'w',padx=2, pady=2)
        ...
        br2 = tkutil.button_row(self.top, ('Generate MAGIC Input', self.Generate_MAGIC_cb),
                                    ...
                                    ('Generate MAUS Input', self.Generate_MAUS_cb),
    ...
    def get_settings(self):
        ...
        for (resi,resn) in self.sequence:
            if resn in self.labeling.get():
                ...
                for me in Me_dic[resn]:
                    ...
                    if resn in ['L', 'V']:
                        if self.dimethyl.state() == True: 
                           if resn == "V":  Vpairs = Vpairs +0.5
                           if resn == "L": Lpairs = Lpairs +0.5
                        if self.proS.state() == False and self.proR.state() == False: 
                           Labels.append(resn + str(resi) + me)
                           pdb_atoms.append(resn + str(resi) +'-' + me.split('-')[0])
                        if self.proS.state() == True: # Leu-D2/Val-G2
                            if me in ['CD2-HD2', 'CG2-HG2']:
                               Labels.append(resn + str(resi) + me)
                               pdb_atoms.append(resn + str(resi) +'-' + me.split('-')[0])
                        if self.proR.state() == True: # Leu-D1/Val-G1
                            if me in ['CD1-HD1', 'CG1-HG1']:
                               Labels.append(resn + str(resi) + me)
                               pdb_atoms.append(resn + str(resi) +'-' + me.split('-')[0])
        ...
        settings.short_noesy_spectrum = self.sc_shortCCH_NOESY.spectrum()
        ...
        settings.TLproR = self.TproR.state()
        settings.TLproS = self.TproS.state()
        return settings
    ...
    def show_summary(self):
        ...
        self.sI, self.sL, self.sV, self.sM, self.sA, self.sT = 0,0,0,0,0,0
        self.uI, self.uL, self.uV, self.uM, self.uA, self.uT = 0,0,0,0,0,0
        self.aI, self.aL, self.aV, self.aM, self.aA, self.aT = 0,0,0,0,0,0
        self.cI, self.cL, self.cV, self.cM, self.cA, self.cT = 0,0,0,0,0,0
        for assignment in s.Labels:
            exec('self.s{:} = self.s{:} + 1'.format(assignment[0],assignment[0]))
        ...
        for peak in s.hmqc_spectrum.peak_list():
            ...
            if len(peak.note) != 0:
                ...
                if len(metype) == 1:
                    if metype in s.labeling:
                        if metype == '-':metype = peak.assignment[0]
                        exec('self.u{:} = self.u{:} + 1'.format(metype,metype))
                        if metype == 'L' and 'C' in peak.note: pairedL += 0.5
                        if metype == 'V'and 'C' in peak.note: pairedV += 0.5
                if len(metype) >= 2:
                    aTotal = aTotal +1
                    for me in metype:
                        exec('self.a{:} = self.a{:} + 1'.format(me,me))
                    if 'L'in metype and 'C' in peak.note: pairedLV += 0.25
                    if 'V'in metype and 'C' in peak.note: pairedLV += 0.25
                if peak.assignment in s.Labels:
                    exec('self.c{:} = self.c{:} + 1'.format(peak.assignment[0],peak.assignment[0]))
        ...
        uTotal = self.uI + self.uL + self.uV + self.uM + self.uA + self.uT
        cTotal = self.cI + self.cL + self.cV + self.cM + self.cA + self.cT
        sTotal = self.sI + self.sL + self.sV + self.sM + self.sA + self.sT

        self.summary_list.append('Number of Methyl Peaks:')
        self.summary_list.append(('Expected: {:3d}'.format(sTotal)))
        self.summary_list.append(('Observed: {:3d}'.format(len(s.hmqc_spectrum.peak_list()))))
        self.summary_list.append(('Overlapped: {:3d}'.format(len(Overlapped))))
        self.summary_list.append(('Assigned: {:3d}'.format(cTotal)))
        self.summary_list.append(('Ambiguously Typed: {:3d}'.format(aTotal)))
        self.summary_list.append('   Expected  Unambiguous Ambiguous Assigned ')
        for me in s.labeling:
            self.summary_list.append('{:}    {:3d}         {:3d}       {:3d}       {:3d}'.format(me, eval('self.s{:}'.format(me)), eval('self.u{:}'.format(me)), eval('self.a{:}'.format(me)), eval('self.c{:}'.format(me))))
        if s.dimethyl == True or s.mono == True:
            self.summary_list.append('Found {:.0f} of {:.0f} possible LV pairs'.format((pairedV+pairedL+pairedLV),s.possible_pairs))
            self.summary_list.append('Found {:.0f} of {:.0f} possible L pairs'.format(pairedL,self.sL/2))
            self.summary_list.append('Found {:.0f} of {:.0f} possible V pairs'.format(pairedV,self.sV/2))
            if pairedLV !=0: self.summary_list.append('Found {:.0f} LV pairs'.format(pairedLV))
        if s.noesy_spectrum.dimension >= 3:
            self.summary_list.append('Found {:d} Isolated methyls'.format(len(Isolated)))
            self.summary_list.append('Experimental NOEs in Cm-CmHm 3D {:d}'.format(len(s.noesy_spectrum.peak_list())))
            self.summary_list.append('   {:d} Reciprocated peaks'.format(len(Reciprocated)))
            if len(Diagonal) > 0:
                self.summary_list.append('   {:d} Diagonal peaks'.format(len(Diagonal)))
            if len(Orphan) > 0:
                self.summary_list.append('   {:d} Orphan peaks'.format(len(Orphan)))
            if len(Bad) > 0:
                self.summary_list.append('   {:d} Bad peaks need removal'.format(len(Bad)))
        if s.pdb_path:
            NOEcount, ULN, LN, Isolated2 = self.Cacl_theo_NOE_networks()
            self.summary_list.append('')
            self.summary_list.append('PDB Statistics using a {:}A cutoff'.format(self.max_dist.get()))
            self.summary_list.append('   Expected # of NOEs in Cm-CmHm 3D {:d}'.format(NOEcount))
            self.summary_list.append('   {:d} of {:d} have local networks'.format(LN, len(s.Labels)))
            self.summary_list.append('   {:d} of {:d} of local networks are unique' .format(ULN, LN))
            self.summary_list.append('   Minimum assignment {:2.1f}%%'.format((float(ULN)/len(s.Labels))*100.0))
            self.summary_list.append('   Maximum assignment {:2.1f}%%'.format((float(LN)/len(s.Labels))*100.0))
            self.summary_list.append('   Found {:d} isolated methyls'.format(len(Isolated2)))
            sx = range(0,len(Isolated2),4)[-1]
            PDBiso = '   '
            for x in range(0, len(Isolated2),4)[:-1]:
                self.summary_list.append('   {:} {:} {:} {:}'.format(Isolated2[x],Isolated2[x+1],Isolated2[x+2],Isolated2[x+3]))
            lx = len(Isolated2) - sx
            for lx in range(lx):
            ...
            if s.Typing_HMQC.name != s.hmqc_spectrum.name:
                ...
                for peak in s.Typing_HMQC.peak_list():
                    ...
                    if len(Matches) >= 1:
                        ...
                        mtype2 = peak2.note.split()[0]
                        if peak2.note.split() and len(peak2.note.split()[0]) < len(mtype):
                            mtype = peak2.note.split()[0]
                        ## get the assignment info:
                        if peak2.assignment in s.Labels and peak.assignment in s.Labels:
                            group = peak2.resonances()[0].group.name
                            atom1 = peak2.resonances()[0].atom.name
                            atom2 = peak2.resonances()[1].atom.name
                        if peak.assignment in s.Labels and peak2.assignment not in s.Labels:
                            group = peak.resonances()[0].group.name
                            atom1 = peak.resonances()[0].atom.name
                            atom2 = peak.resonances()[1].atom.name
                        if peak.assignment not in s.Labels and peak2.assignment in s.Labels:
                            group = peak2.resonances()[0].group.name
                            atom1 = peak2.resonances()[0].atom.name
                            atom2 = peak2.resonances()[1].atom.name
                        if peak.assignment not in s.Labels and peak2.assignment not in s.Labels:
                            group = "{:}{:}".format(mtype.lower(), peak2.resonances()[0].group.number)
                            atom1 = peak2.resonances()[0].atom.name
                            atom2 = peak2.resonances()[1].atom.name
                        if mtype == 'L' and s.TLproR == True:
                            atom1 = 'CD1'
                            atom2 = 'HD1'
                        if mtype == 'L' and s.TLproS == True:
                            atom1 = 'CD2'
                            atom2 = 'HD2'
                        if mtype == 'V' and s.TLproR == True:
                            atom1 = 'CG1'
                            atom2 = 'HG1'
                        if mtype == 'V' and s.TLproS == True:
                            atom1 = 'CG2'
                            atom2 = 'HG2'
                        self.Assign_HMQC(peak, group, atom1, atom2)
                        self.Assign_HMQC(peak2, group, atom1, atom2)
                        if not re.search('[L,V]', peak2.note):peak2.label.color = 'white'
                        retyped.append(peak2)
                        if 'C' in peak2.note: 
                            peak2type = peak2.note.split()[0]
                            if s.TLproR == True:
                                peak2.note  = "{:} ;{:}{:}-{:}".format(mtype, group,atom1.replace('1','2'), atom2.replace('1','2'))
                            if s.TLproS == True:
                                peak2.note  = "{:} ;{:}{:}-{:}".format(mtype, group,atom1.replace('2','1'), atom2.replace('2','1'))
                ...
                for peak3 in s.hmqc_spectrum.peak_list():
                    if peak3 not in retyped:
                        ...
                        if s.TLproR == False and s.TLproS == False:
                        ...
                        if peak3.note.split():
                            group = peak3.resonances()[0].group.name
                            if peak3.note.split()[0] == 'L' and s.TLproR == True:
                                self.Assign_HMQC(peak3, group, 'CD2', 'HD2')
                                peak3.note = "L ;{:}CD1-HD1".format(group)
                            if peak3.note.split()[0] == 'L' and s.TLproS == True:
                                self.Assign_HMQC(peak3, group, 'CD1', 'HD1')
                                peak3.note = "L ;{:}CD2-HD2".format(group)
                            if peak3.note.split()[0] == 'V' and s.TLproR == True:
                                self.Assign_HMQC(peak3, group, 'CG2', 'HG2')
                                peak3.note = "V ;{:}CG1-HG1".format(group)
                            if peak3.note.split()[0] == 'V' and s.TLproS == True:
                                self.Assign_HMQC(peak3, group, 'CG1', 'HG1')
                                peak3.note = "V ;{:}CG2-HG2".format(group)
    ...
    def Geminal_Pairs_cb(self):
        ...
        if s.HMBC_spec.dimension != 3:
            tkMessageBox.showinfo('Input Error', "Please Select 3D HMBC-HMQC Spectrum")
            return
            ...
            if x not in Paired: 
                ...
                if len(HMBC_A) >=1:
                    ...
                    for y in Me2list:
                        ...
                        message = ('Checking possible combination {:d} \nFound {:d} of possible geminal pairs {:d}'.format(self.count, self.LVcount, s.possible_pairs))
                        self.progress_report(message)
    ...
    def Generate_MAGIC_cb(self):
        ...
        for x in range(len(HMQCpeaks)):
            ...
            HMCQnew.write("  {:>14} {:10.3f} {:10.3f}\t\t{:}\n".format(passign, peak.frequency[0],peak.frequency[1],note))
        HMCQnew.close()
        ...
        for noe in s.noesy_spectrum.peak_list():
            CCHnoesy.write("{:17} {:10.3f} {:10.3f} {:10.3f}   {:}\n".format('?-?-?',noe.frequency[0], noe.frequency[1], noe.frequency[2], noe.data_height))
        CCHnoesy.close()
    ...
    # ------------------------------------------------------------------------------
    # 
    def Generate_MAUS_cb(self):

        s = self.get_settings()
        outdir = self.session.project.sparky_directory + '/' + self.session.project.save_path.split('/')[-2] + '/MAUS/'
        if not os.path.exists(outdir):
            os.makedirs(outdir)

        HMQCpeaks = sorted(s.hmqc_spectrum.peak_list(), key = lambda x: (x.assignment, x.frequency[0]))
        HMCQnew = open(outdir + s.hmqc_spectrum.name + '_maus.list', 'w')
        HMCQnew.write("{:^17} {:^10} {:^10}   {:16}\n".format('Assignment','w1','w2','Data Height'))
        for x in range(len(HMQCpeaks)):
            peak = HMQCpeaks[x]
            note = peak.note
            if len(note) == 0:
            ...
            if '?' not in peak.assignment:
                if s.rename == False:
                    group = "{:}X{:}".format(peak.resonances()[0].group.symbol.upper(),peak.resonances()[0].group.number)
                if s.rename == True:
                    if peak.assignment not in s.Labels:
                        group = "{:}X{:}".format(peak.resonances()[0].group.symbol.upper(),peak.resonances()[0].group.number)
                    if peak.assignment in s.Labels:
                        group = peak.resonances()[0].group.name
                if 'L' in note or 'V' in note:
                    group = group + '?'
            HMCQnew.write("{:>17} {:>10.3f} {:>10.3f}   {:.0f} \n".format(group, peak.frequency[0],peak.frequency[1],peak.data_height))
        HMCQnew.close()

        CCHnoesy = open(outdir + s.noesy_spectrum.name + '.list','w')
        CCHnoesy.write("{:>17} {:^10} {:^10} {:^10}   {:}\n".format('Assignment','w1','w2','w3','Data Height'))
        for noe in s.noesy_spectrum.peak_list():
            CCHnoesy.write("{:>17} {:>10.3f} {:>10.3f} {:>10.3f}   {:.0f} \n".format('?-?-?',noe.frequency[0], noe.frequency[1], noe.frequency[2], noe.data_height))
        CCHnoesy.close()

        shortCCHnoesy = open(outdir + s.short_noesy_spectrum.name + '.list','w')
        shortCCHnoesy.write("{:>17} {:^10} {:^10} {:^10}   {:}\n".format('Assignment','w1','w2','w3','Data Height'))
        for noe in s.short_noesy_spectrum.peak_list():
            shortCCHnoesy.write("{:>17} {:>10.3f} {:>10.3f} {:>10.3f}   {:.0f} \n".format('?-?-?',noe.frequency[0], noe.frequency[1], noe.frequency[2], noe.data_height))
        shortCCHnoesy.close()
    ...
    def make_methylFLYA_cb(self):
        ...
        for x in range(len(HMQCpeaks)):
            ...
            if peak.is_assigned != 0:
                ...
                if len(peak.note) < 1: 
                    tkMessageBox.showinfo('Input Error', " Missing Peak Type\nfor {:}\nUpdate list try again".format(peak.assignment))
                    return
                ...
                n = x + 1
                HMQCnew.write("{:4.0f}{:8.3f}{:8.3f} 1 U{:19.3e}         0 e   0  {:8}  {:8}\n".format(n, peak.frequency[0],peak.frequency[1], peak.data_height, trans[0], trans[0].replace('C','Q')))
        ...
        for y in range(len(LVpeaks)):
            ...
            HMBCout.write("{:4.0f}{:8.3f}{:8.3f}{:8.3f}{:8.3f} 1 U   1.000000E+00  0.000000E+00 e 0    {:8}   {:8}   {:8}   {:8}\n".format(n, peak.frequency[0],peak.frequency[1], Freqs[pair][0], Freqs[pair][1], LVIDs[peak.assignment], LVIDs[peak.assignment].replace('C','Q'),LVIDs[pair], LVIDs[pair].replace('C','Q')))
        ...
        for noe in s.noesy_spectrum.peak_list():
            n = n +1 
            CCHnoesy.write("{:4.0f}{:8.3f}{:8.3f}{:8.3f} 1 U{:19.3e}         0 e   0 -         -         -\n".format(n,noe.frequency[0], noe.frequency[1], noe.frequency[2], noe.data_height))
        CCHnoesy.close()
